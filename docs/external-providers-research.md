...
Dynamic site scraping deferred – Sites que exigem scraping pesado ou login (Programathor, Remotar, Findwork, Jobdata, Vagas Aggregator) devem ser postergados para fases futuras, pois demandam navegador headless, bypass de Cloudflare ou credenciais. Focar em APIs públicas reduzirá a manutenção e riscos de bloqueio.
