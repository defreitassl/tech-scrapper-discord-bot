...
- Vagas coletadas que passam pelo filtro de qualidade recebem prioridade em memoria antes de salvar. A prioridade favorece estagio remoto em tecnologia, estagio em Minas Gerais/BH/regiao, trainee remoto e junior remoto, mas vagas `LOW`, junior, presenciais ou hibridas boas continuam podendo ser salvas como `DRAFT`.
- `priority` e `score` ainda nao existem no banco. Nao ha migration nem persistencia de prioridade nesta etapa; o diagnostico fica em logs/toasts e no resumo retornado pelo runner.
...
- A prioridade de providers fica em `src/services/jobPriority.ts`; ela e deterministica, nao usa IA, roda depois da qualidade e antes da deduplicacao/escrita, ordena `HIGH` > `MEDIUM` > `LOW` preservando ordem original dentro do mesmo nivel, e nao bloqueia vagas `LOW`.
...
- A listagem possui a acao `Coletar Remotar`, que chama `POST /admin/jobs/collect-remotar` e executa apenas `remotarProvider` pelo mesmo lock. Esta coleta e experimental/manual e nao entra na coleta automatica.
...
- O provider experimental Remotar e `src/providers/remotar.provider.ts`, documentado em `docs/remotar-scraping-research.md`.
...
- Vagas aceitas por qualidade geram log `Prioridade calculada para vaga coletada` com `priority`, `score` e `reasons`. O runner contabiliza prioridades criadas em `highPriority`, `mediumPriority` e `lowPriority`.
...
- Nao ha scraping HTML real de fontes protegidas, Cheerio, LinkedIn ou Solides nesta etapa. A Gupy existe apenas como provider experimental manual baseado em endpoint publico observado, com limite baixo e fora da coleta automatica. O Programathor existe apenas como provider experimental manual por HTML publico simples, com limite baixo e fora da coleta automatica. A Remotar existe apenas como provider experimental manual por JSON publico observado com Playwright MCP, com limite baixo e fora da coleta automatica. Playwright foi usado no reconhecimento e tambem esta instalado como infraestrutura isolada e smoke test nao registrado.
...
- A revisao operacional dos providers experimentais fica em `docs/experimental-providers-operational-review.md`. Em 2026-05-22, Remotar foi o melhor candidato para automacao futura (20 retornadas, 18 passariam pelo filtro de qualidade), Gupy teve menor volume mas boa qualidade (10 retornadas, 10 passariam), e Programathor teve baixo rendimento (2 retornadas, 2 passariam) por muitas vagas antigas/vencidas. Mesmo assim, nenhum experimental deve entrar na coleta automatica sem nova decisao explicita.
...
- Nao executa `experimentalJobProviders` automaticamente nesta etapa. Isso inclui Gupy, Programathor e Remotar.
