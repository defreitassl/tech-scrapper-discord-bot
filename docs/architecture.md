...
- `src/services/autoApproveJobs.ts`: rotina legada para processar vagas `DRAFT` antigas. O fluxo principal de coleta nao usa mais esse status, mas o botao de rascunhos legados ainda pode gerar `aiGeneratedText` e mover vagas antigas para `PENDING`.
...
11. Para vagas `DRAFT` legadas, o admin pode usar `Preparar rascunho legado`. Essa acao chama Gemini, salva `aiGeneratedText`, marca `useAi = true` e altera o status para `PENDING` somente se a IA gerar a mensagem com sucesso. A acao nao envia a vaga ao Discord.
...
13. Se a vaga ja estiver `PENDING`, a acao manual de preparo pode regenerar `aiGeneratedText` e manter a vaga na fila. Vagas `SENT` ou `ARCHIVED` nao sao preparadas.
...
17. `discordPublisher` envia a mensagem ao canal configurado por `DISCORD_CHANNEL_ID`.
18. A vaga e marcada como `SENT` com `sentAt`.
19. Em caso de erro, a vaga e marcada como `ERROR`.
...
14. O runner seleciona apenas a quantidade necessaria para completar o limite diario do scheduler, descontando `SENT` hoje e `PENDING` atuais.
15. Para cada selecionada, o runner gera `aiGeneratedText` com Gemini e cria o registro como `PENDING`, com `useAi = true`.
16. Se a IA falhar, a vaga e recusada e nao e persistida.
17. A coleta manual nao envia nada ao Discord.
18. O painel redireciona de volta para `/admin/jobs` com um toast compacto: `Coleta concluida: X aprovadas para envio, Y recusadas, Z duplicatas, W erros.`
...
Depois da coleta, o runner automatizado normaliza, aplica filtro de dominio e qualidade, deduplica, calcula prioridade, seleciona pelo limite diario, gera mensagem com IA e cria registros como `PENDING`. Vagas rejeitadas por qualidade, dominio, prioridade, duplicidade ou falha de IA nao sao persistidas.

A coleta GitHub chama IA apenas para vagas selecionadas e aprovadas, nao envia vagas ao Discord e nao altera o agendamento de envio. O terminal registra um resumo estruturado por repositorio e logs das rejeicoes com `reasons`. Nao ha tabela, pagina ou persistencia de logs de coleta.
...
Depois desse filtro, o mesmo runner central normaliza, aplica `evaluateCollectedJobQuality`, deduplica, prioriza, gera IA para as selecionadas e cria registros como `PENDING`. A coleta externa nao envia vagas ao Discord.
...
Depois desses filtros, o runner central normaliza, aplica `evaluateCollectedJobQuality`, deduplica, prioriza, gera IA para as selecionadas e cria registros como `PENDING`. A coleta ATS nao envia vagas ao Discord e nao altera schema Prisma.
...
Depois dos filtros do provider, o runner central normaliza, aplica qualidade, deduplica, prioriza, gera IA para as selecionadas e cria registros como `PENDING`. A coleta Gupy nao envia ao Discord.
...
Depois dos filtros do provider, o runner central normaliza, aplica qualidade, deduplica, prioriza, gera IA para as selecionadas e cria registros como `PENDING`. A coleta Programathor nao envia ao Discord.
...
Depois dos filtros do provider, o runner central normaliza, aplica qualidade, deduplica, prioriza, gera IA para as selecionadas e cria registros como `PENDING`. A coleta Remotar nao envia ao Discord.
...
Ela executa os providers registrados em `automaticJobProviders`: GitHub, Himalayas, Jobicy, RemoteOK, Remotive, Remotar, Gupy e Programathor. Os providers ATS ficam em `atsJobProviders` e nao entram na coleta automatica nesta etapa.
...
- salva `useAi = true` e `aiGeneratedText`;
- chama Gemini apenas para candidatas selecionadas;
- nao envia ao Discord;
- reaproveita normalizacao, filtro de dominio, filtro de qualidade, deduplicacao e prioridade.

Antes de coletar, `runAutomatedJobCollection` calcula quantas vagas ainda podem ser criadas: `dailyLimit - SENT hoje - PENDING atuais`. Se o resultado for zero, nenhuma vaga e coletada/criada. Se houver espaco, o runner seleciona as melhores candidatas ate o limite necessario; falha de IA recusa a vaga sem criar `DRAFT`.
...
Nas coletas de providers, essas notificacoes sao deliberadamente compactas. O painel mostra aprovadas, recusadas, duplicatas e erros; detalhes como descartes por data, senioridade, localizacao, qualidade, prioridade, duplicidade e IA continuam no terminal.
